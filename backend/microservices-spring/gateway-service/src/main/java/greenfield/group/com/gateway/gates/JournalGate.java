...
public class JournalGate extends Gate {
     ...
     * @return
     */
    @RequestMapping(path = "/journal-gate/{methodName}", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public SimpleResult postLoad(@RequestHeader(value = "Authorization", defaultValue = "") String authorization
        ...
        return this.restTemplate
                .postForEntity(serviceRegistry.get(JOURNAL_SERVICE) + "/" + methodName, httpEntity, SimpleResult.class)
                .getBody();
    }
     ...
     * @return
     */
    @RequestMapping(path = "/journal-gate/{methodName}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
